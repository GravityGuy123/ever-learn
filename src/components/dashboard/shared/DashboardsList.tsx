...
export default function DashboardsList() {
  ...
  const hasStudent = Boolean((user as any)?.is_student);
  const hasTutor = Boolean((user as any)?.is_tutor);
  const hasModerator = Boolean((user as any)?.is_moderator);
  const hasAdmin = Boolean((user as any)?.is_admin || (user as any)?.is_staff);
